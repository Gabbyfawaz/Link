...
//  Link
//
//  Created by Gabriella Fawaz on 2021/09/21.
//
...
//
     ...
     override init(frame: CGRect) {
         ...
         mapsButton.addTarget(self, action: #selector(didTapGoogleMaps), for: .touchUpInside)
     ...
     override func layoutSubviews() {
         ...
         locationButton.frame = CGRect(x: contentView.width-15-20 , y: 0, width: 20, height: 20)
         
         let mapHeight = height-20-20
         let mapWidth = contentView.width-10-10-mapHeight/4
         mapView.frame = CGRect(x: 15,
                                y: titleLabel.height+10,
                                width: contentView.width-30-15-mapHeight/4.5,
                                height:mapHeight)
         stackView.frame = CGRect(x: mapView.right+15,
                                  y: mapView.top+30,
                                  width: mapHeight/4.5,
                                  height: mapHeight-60)
         customLocationView.frame = CGRect(x: (mapWidth-(contentView.width-150))/2,
                                           y: titleLabel.height+10 + (mapHeight-80)/2,
 ...
 //        annotationView?.frame.size = CGSize(width: 45, height: 45)
     ...
     func configure(with viewModel: PostLocationCollectionViewCellViewModel) {
         ...
         guard let locationString = viewModel.location else {
             return
         }
         ...
         StorageManager.shared.profilePictureURL(for: viewModel.user, completion: { url in
             ...
             self.linkIconImageView.sd_setImage(with: URL(string: viewModel.postLinkIcon) , completed: nil)
         })
...
extension PostLocationCollectionViewCell: CustomLocationPopUpDelegate {
