...
class PostEditViewController: UIViewController, UICollectionViewDelegate, UICollectionViewDataSource {
    ...
    @objc func didTapNext() {
        ...
        if navigationController?.navigationBar.backItem?.title == "Take Photo" {
            let vc = CaptionViewController(image: current)
            vc.title = "Add Caption"
            navigationController?.pushViewController(vc, animated: true)
        } else {
//            let vc = CreateLinkViewController(image: current)
            
            let vc = LocationViewController(image: current)
            navigationController?.pushViewController(vc, animated: true)
        }
    }
