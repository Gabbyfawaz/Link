...
//    func updateMapViewControllerUI(_ vc: PinMapViewController)
//}
  ...
  private var mapView = MKMapView()
    ...
    @objc private func didTapDone() {
        ...
        guard let username = UserDefaults.standard.string(forKey: "username"),
              ...
              let longitude = mapItems?.first?.placemark.coordinate.longitude else {
            return
        }
        
        let coordinates = Coordinate(latitude: latitude, longitude: longitude)
        
        let usersBoom = SearchUser(name: username)
        let pin = Pin(people: [usersBoom], cooordinates: coordinates, locationString: name, timeStamp: timeStamp)
   
        DatabaseManager.shared.createBoomingPin(pin: pin) { success in
            /// we want to add dismiss to root controller
            if success {
//                delegate.updateMapsViewControllerUI(self)
                NotificationCenter.default.post(name: .didUpdatePins, object: nil)
