        ...
        guard let email = UserDefaults.standard.string(forKey: "email"),
              let username = UserDefaults.standard.string(forKey: "username") else {
                  AuthManager.shared.signOut { success in
                      if success {
                          print("Sucessfully signed out of the account")
                          let vc = SignInViewController()
                          self.navigationController?.pushViewController(vc, animated: true)
                      } else {
                          fatalError("Email and username is nil")
                      }
                  }
            return
        }
...
//        let profile = LinkNotificationViewController()
...
//            nav3.navigationItem.backButtonTitle = ""
        ...
        // Define tab items
//        nav1.tabBarItem = UITabBarItem(title: "Profile", image: UIImage(systemName: "person"), tag: 1)
